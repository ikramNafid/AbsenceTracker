...
class DatabaseHelper {
  ...
  Future<Database> _initDatabase() async {
    String path = join(await getDatabasesPath(), 'absence_tracker.db');
    return await openDatabase(
      path,
      version: 1,
      onCreate: (db, version) async {
        // Appel des fonctions de création des tables
        await GroupTable.create(db);
        await StudentTable.create(db);
        await ModuleTable.create(db);
        await SessionTable.create(db);
        await AbsenceTable.create(db);
      },
    );
  }
...
// import 'package:sqflite/sqflite.dart';
// import 'package:path/path.dart';
// import 'package:absence_tracker/database/models/session_model.dart';
